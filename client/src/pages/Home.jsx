...
const Home = () => {
  ...
  const handleIsUserExist = async (e) => {
    ...
    const isUserExist = await dispatch(IS_USER_EXIST({ email: e.target.email.value }));
    if (isUserExist?.payload?.data?.isUserExist) {
      navigate(`/signin/${e?.target?.email?.value}`); //if user exists already redirect to signin
    } else {
      navigate("/signup");
    }
  };
