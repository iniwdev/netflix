...
const UserExistLogin = () => {
  ...
  const handleSignIn = async (e) => {
    ...
    const response = await dispatch(SIGN_IN({ email, password }));
    const userData = response.payload;

    if (response.meta.requestStatus === "fulfilled" && userData?.success) {
      toast.success("Signed in successfully!");
      return navigate("/browse");
    } else {
      toast.error(response.payload || "Incorrect password or invalid credentials");
    }
  };
  ...
  return (
    <SignUpLayout>
      <div className="flex w-full items-center justify-center">
        <form
          className="my-20 max-w-[440px] p-3 shadow-lg shadow-gray-200"
